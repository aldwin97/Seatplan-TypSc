...
function SeatPopup({
  ...
  function getUpdatedByFromSessionStorage(): number {
    ...
    if (user_id) {
      console.log(
        "Value of updated_by retrieved from session storage:",
        user_id
      );
      return parseInt(user_id, 10);
    } else {
      console.warn(
        "Value of updated_by not found in session storage. Defaulting to 0."
      );
      return 0;
    }
  }
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Check if the selected occupant is already assigned to another seat
    const isOccupantAssigned = seats.some(
      (s) => s.occupant === selectedOccupant && s.seat_id !== seat.seat_id
    );
    ...
    try {
      ...
      if (response.ok) {
        ...
        const unassignedOccupants = occupantsData.filter((occupant) => {
          return !seats.some(
            (s) => s.occupant === occupant.user_id.toString() && s.seat_id
          );
        });
...
const SeatPopupComments = ({ userId, seatIds }: SeatPopupCommentsProps) => {
  ...
  const [replyToRecipientId, setReplyToRecipientId] = useState<number | null>(
    null
  );
    ...
    return fetch(`http://localhost:8080/admin/showAllCommentBy/${seatId}`)
      .then((response) => {
        if (response.ok) {
          ...
          console.error(
            "Failed to fetch comments for seat:",
            response.statusText
          );
          return [];
        }
  ...
  const renderMainComments = (comments: Comment[], userId: number) => {
    ...
    const findReplyingTo = (commentId: number): string | undefined => {
      ...
      return originalComment
        ? findFullNameById(originalComment.user_id)
        : undefined;
    };

    const findReplyToUser = (
      commentId: number,
      replies: Comment[]
    ): Comment | undefined => {
      for (const reply of replies) {
              ...
              <td>
                ...
                {comment.parent_id && (
                  <div className={styles.repliedTo}>
                    {findFullNameById(comment.user_id)} Replied to{" "}
                    {findFullNameById(comment.recipient_id)}
                  </div>
                )}
              </td>
              <td>
                ...
                {comment.user_id === userId &&
                  comment.recipient_id !== userId && (
                    <div className={styles.repliedTo}>
                      You replied to{" "}
                      {findFullNameById(comment.recipient_id) || "this seat"} ↷
                    </div>
                  )}
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}>
              ...
              {/* Use replyToName instead of userId */}
              <button
                className={styles.closeButton2}
            ...
            <div className={styles.modalBody}>
              ...
              <button
                ...
                onClick={() => handleReplySubmit(seatIds[0])}
              >
                Reply
              </button>
            </div>
    ...
    if (canvas && ctx) {
      ...
      seats.forEach((seat) => {
        ...
        if (seat.isSwapping) {
          ...
          const swappedSeat = seats.find(
            (s) => String(s.seat_id) === String(seat.occupant)
          );
        ...
        if (seat.position_name) {
          ...
          if (seat.position_name === "Machine") {
            ...
            machineIcon.onload = () => {
              ...
              ctx.drawImage(
                machineIcon,
                machineIconX - machineIconSize / 4,
                machineIconY - machineIconSize / 2.5,
                machineIconSize,
                machineIconSize
              );
            };
              ...
              {showInfoGuide && (
                <div className={styles["infoguide"]}>
                  ...
                  <span
                    ...
                    onClick={handleInfoGuideClose}
                  >
                    &times;
                  </span>
                  <h2>Information Guide</h2>
                  <p>This is a simple guide on how to use the page.</p>
                  <br />
                  <p>
                    • Use the middle mouse hold and drag to move around the
                    canvas of the page.
                  </p>
                  <p>
                    • To open and view the seats information, point the mouse on
                    the seat number and double left click.
                  </p>
                  <p>
                    • On Edit Mode, there are two things you can do, assigning
                    an occupant to the current selected seat or swap the current
                    user on another user on a different seat.
                  </p>
                  <p>
                    • When swapping seats, you can select the seat to be swap on
                    the current seat you are editing.
                  </p>
                </div>
              )}
            </>
